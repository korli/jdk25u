    ...
    const char* filetype)
{
    ...
    bool accept = env->CallBooleanMethod(fPeer, acceptFileMethod, javaString);
...
Java_sun_hawt_HaikuFileDialog_initIDs(JNIEnv *env, jclass clazz)
{
    acceptFileMethod = env->GetMethodID(clazz, "acceptFile", "(Ljava/lang/String;)Z");
    doneMethod = env->GetMethodID(clazz, "done", "([Ljava/lang/String;)V");
}
