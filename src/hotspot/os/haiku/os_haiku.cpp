...
bool os::available_memory(physical_memory_size_type& value) {
  system_info si;
  if (get_system_info(&si) != B_OK) return false;
  value = (physical_memory_size_type)(si.max_pages - si.used_pages) * vm_page_size();
  return true;
}
...
void os::Haiku::initialize_system_info() {
  ...
  if (result != B_OK) {
    fprintf(stderr, "get_system_info failed: %d\n", result);
...
static void *thread_native_entry(Thread *thread) {
  ...
  log_info(os, thread)("Thread is alive (tid: " "%" PRIuPTR ", pthread id: " "%" PRIuPTR ").",
  ...
  log_info(os, thread)("Thread finished (tid: " "%" PRIuPTR ", kernel thread "
    "id: " INT32_FORMAT ").", os::current_thread_id(), find_thread(NULL));
...
bool os::create_thread(Thread* thread, ThreadType thr_type,
...

  {
    ...
    char buf[64];
    if (ret == 0) {
      log_info(os, thread)("Thread \"%s\" started (pthread id: " "%" PRIuPTR ", attributes: %s). ",
...
bool os::create_attached_thread(JavaThread* thread) {
  ...
  log_info(os, thread)("Thread attached (tid: " "%" PRIuPTR ", pthread id: " "%" PRIuPTR
                       ", stack: " PTR_FORMAT " - " PTR_FORMAT " (" "%zu" "K) ).",
                       os::current_thread_id(), (uintx) pthread_self(),
...
bool os::total_swap_space(physical_memory_size_type& value) {
  system_info si;
  if (get_system_info(&si) != B_OK) return false;
  value = (physical_memory_size_type)si.max_swap_pages * vm_page_size();
  return true;
}
...
bool os::free_swap_space(physical_memory_size_type& value) {
  system_info si;
  if (get_system_info(&si) != B_OK) return false;
  value = (physical_memory_size_type)si.free_swap_pages * vm_page_size();
  return true;
}
...
bool os::start_debugging(char *buf, int buflen) {
  ...
  jio_snprintf(p, buflen-len,
               ...
               "To debug, run 'gdb /proc/%d/exe %d'; then switch to thread " "%" PRIuPTR " (" INTPTR_FORMAT ")\n"
